...
const {isAuth, user,fetchBlog, savedBlog,getSavedBlogs} = useAppData();
...
setLoading(false)
  ...
  async function deleteBlog() {
   ...
   setLoading(true);
      const token = Cookies.get("token");
      const {data} = await axios.delete(`${author_service}/api/v1/blog/delete/${id}`,{
        headers:{
          Authorization:`Bearer ${token}`
        }
      })
...
useEffect(()=>{
  if(savedBlog && savedBlog.some((b)=>b.blogid===id)){
setSaved(true)
  }else{
    setSaved(false)
  }
